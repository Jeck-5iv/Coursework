...
def timeline_creation(n):
    ...
    for i in range(n):
        ...
        now['pipe_destroyed_this_month'] = False
...
def timeline_pipe_build_update(timeline, pipe_build_year, pipe_build_month, pipe_stats):
    timeline_pointer = (pipe_build_year - 2022) * 12 + pipe_build_month
...
def uplifts_reconed_update(timeline, uplifts, pipe_build_year, pipe_build_month):
    pipe_build_timeline_pointer = (pipe_build_year - 2022) * 12 + pipe_build_month
...
def simulation_cars_pipe(timeline, uplifts, pipe_build_year, pipe_build_month, tax_stats, pipe_stats, cars_stats, recon_teams_free_time, first_free_recon_team_pointer, last_well_build_and_recon_finish_time):
    ...
    for year in range(2022, 2051):
        ...
        for month in range(0, 12):
            ...
            timeline_pointer = (year - 2022) * 12 + month
            ...
            if timeline_pointer > last_well_build_and_recon_finish_time and oil_avalible == 0 and delivery_type == 'pipe':
                ...
                if timeline_pointer > 0 and not timeline[timeline_pointer]['pipe_destroyed']:
                    now['pipe_destroyed_this_month'] = True
    ...
    if print_algo_flag and best_method == 'cars_pipe':
        algoritm_printer(timeline)
    return npv
...
def simulation_cars_cars(timeline, uplifts, pipe_build_year,
    ...
    for year in range(2022, 2051):
        ...
        #print(npv, losses, 'npv, losses')
    if print_algo_flag and best_method == 'cars_cars':
        algoritm_printer(timeline)
    return npv
...
def simulation_cars_none(timeline, uplifts, pipe_build_year, pipe_build_month, tax_stats, cars_stats, recon_teams_free_time, first_free_recon_team_pointer):
    ...
    oil_avalible = 0
    npv = 0
    for year in range(2022, pipe_build_year + 1):
        fcf = 0
        ...
        #print(npv, losses, 'npv, losses')
    #printer(timeline)
    if print_algo_flag and best_method == 'cars_none':
        algoritm_printer(timeline)
    return npv
...
def point_simulation(uplifts_count, uplifts, pipe_build_year, pipe_build_month, tax_stats, pipe_stats, cars_stats, well_stats, recon_results, more_simulations_flag, print_npv_flag):
    # помечаем участок таймлайна, который надо дозаполнить
    pipe_build_timeline_pointer = (pipe_build_year - 2022) * 12 + pipe_build_month
    start_pointer = 0
    ...
    average_npv_dict = dict()
    for sim_type in sum_npv.keys():
        average_npv_dict[sim_type] = sum_npv[sim_type] / sim_count

    if print_npv_flag:
        print(average_npv_dict)
    ...
    if points['cars_none'] == max_points:
        ...
        return 'cars_none', average_npv_dict['cars_none']
    elif points['cars_cars'] == max_points:
        average_npv = sum_npv['cars_cars']
        #print('Keep using cars delivery and do not build a pipiline in the', pipe_build_year, 'year in the',
              #pipe_build_month, 'month')
        #print('project emv = ', average_npv)
        return 'cars_cars', average_npv_dict['cars_cars']
    elif points['cars_pipe'] == max_points:
        average_npv = sum_npv['cars_pipe']
        #print('The construction of the pipeline should begin in the', pipe_build_year, 'year in the', pipe_build_month,
              #'month')
        #print('project emv = ', average_npv)
        return 'cars_pipe', average_npv_dict['cars_pipe']
    #return sum_npv

def algoritm_printer(timeline):
    printer(timeline)
    global print_algo_flag
    print_algo_flag = False

    for timeline_pointer in range(len(timeline)):
        year = 2022 + (timeline_pointer // 12)
        month = timeline_pointer % 12
        events = timeline[timeline_pointer]
        if len(events['recon_started']) != 0:
            print('In ', year, ' year ', month, ' month start recon ', end = '')
            for well_id in events['recon_started']:
                print(well_id, end = ', ')
            print('wells')
        if len(events['build_started']) != 0:
            print('In ', year, ' year ', month, ' month start build ', end = '')
            for well_id in events['build_started']:
                print(well_id, end = ', ')
            print('wells')
        if events['pipe_build_started_this_month']:
            print('In ', year, ' year ', month, ' month start build pipeline')
        if events['pipe_destroyed_this_month']:
            print('In ', year, ' year ', month, ' month destroy pipeline')
    return
...
def main(file_name):
    ...
    uplifts_count, recon_stats, uplifts, recon_stats, well_stats, pipe_stats, cars_stats, tax_stats, oil_stats = data_input(
    data_frame)

    # введем текущий год и месяц
    now_year = 2022
    now_month = 0

    #введем текущие результаты разведки
    recon_results = recon_results_create(uplifts)
    print(recon_results)

    # раз проект еще не закрыт и мы еще не возим трубопроводом, значит все это время возили машинами
    # пометим скважины, которые мы уже разведали по нашему алгоритму
    delivery_type = 'cars'
    uplifts = choose_wells_to_recon(uplifts, delivery_type)
    uplifts = recon_rating_create(uplifts, delivery_type)

    # создаем timeline, только потому что мой reconed_update без него не работает
    timeline = timeline_creation(12 * 30)

    # пометим те поднятия, которые на момент принятия решения мы разведали, чтоб не менять число нефти в них при броске точек
    # помечаем, если уже получили результат разведки
    uplifts = uplifts_reconed_update(timeline, uplifts, now_year, now_month)
    printer(uplifts)

    #сначала найдем оптимальную точку принятия решения. потом уточним метод
    max_average_npv = -10 ** 9
    point = [0, -1]
    method = 'None'
    ...
    for i in range(sim_count):
        ...
        for pipe_build_year in range(2022, 2041):
            for pipe_build_month in range(0, 12):
                now_method, now_average_npv = point_simulation(uplifts_count, copy.deepcopy(uplifts), pipe_build_year,
                                                               pipe_build_month, tax_stats, pipe_stats, cars_stats,
                                                               well_stats, copy.deepcopy(recon_results), False, True)
                print([pipe_build_year, pipe_build_month], now_method, now_average_npv)
                #print(now_method, now_average_npv, pipe_build_year, pipe_build_month)
                if now_average_npv > max_average_npv:
                    max_average_npv = now_average_npv
                    point = [pipe_build_year, pipe_build_month]
                    method = now_method
        print(point, method, max_average_npv)

    method = 'None'
    pipe_build_year = point[0]
    pipe_build_month = point[1]
    method, average_npv = point_simulation(uplifts_count, copy.deepcopy(uplifts), pipe_build_year,
                                                   pipe_build_month, tax_stats, pipe_stats, cars_stats,
                                                   well_stats, copy.deepcopy(recon_results), True, True)

    #теперь напечатаем алгоритм действий
    print_algo_flag = True
    best_method = method
    method, nothing = point_simulation(uplifts_count, copy.deepcopy(uplifts), pipe_build_year,
                                           pipe_build_month, tax_stats, pipe_stats, cars_stats,
                                           well_stats, copy.deepcopy(recon_results), False, False)


    #printer(timeline)
    print()
    print(point)
    print(method, average_npv)


file_name = 'Датасет.xlsx'
print_algo_flag = False
best_method = 'None'
main(file_name)
