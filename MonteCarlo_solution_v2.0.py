...
def point_simulation(uplifts_count, uplifts, pipe_build_year, pipe_build_month, tax_stats, pipe_stats, cars_stats, well_stats, recon_results, last_one_flag):
    # помечаем участок таймлайна, который надо дозаполнить
    pipe_build_timeline_pointer = (pipe_build_year - 2021) * 12 + pipe_build_month
    start_pointer = 0
...
def main(file_name):
    ...
    #будем генерировать recon_results и для каждого считать оптимальный вариант при действии нашим алгоритмом с помощью монтекарло
    #такой вот монтекарло в монтекарло\\
    #хотим найти пару точка предположительной постройки + метод

    candidates_mass = []
    sim_count = 2

    for i in range(sim_count):
        recon_results = recon_results_create(copy.deepcopy(uplifts))
        print(recon_results)

        max_average_npv = -10 ** 9
        point = [2019, -1]
        method = 'None'
        for pipe_build_year in range(2020, 2041):
            for pipe_build_month in range(0, 12):
                now_method, now_average_npv = point_simulation(uplifts_count, copy.deepcopy(uplifts), pipe_build_year,
                                                               ...
                                                               well_stats, copy.deepcopy(recon_results), False)
                ...
                if now_average_npv > max_average_npv:
                    ...
                    method = now_method
        candidates_mass.append(point)
        print(point, method, max_average_npv)
    print(candidates_mass)

    points_mass_for_points = [0 for i in range(len(candidates_mass))]
    max_average_npv = -10 ** 9
    point = [2019, -1]
    method = 'None'
    ...
    for i in range(sim_count):
        ...
        for point in candidates_mass:
            ...
            if now_average_npv > max_average_npv:
                ...
                point = [pipe_build_year, pipe_build_month]
                method = now_method
        points_mass_for_points[candidates_mass.index(point)] += 1


    print(points_mass_for_points)
    final_point = candidates_mass[points_mass_for_points.index(max(points_mass_for_points))]
    print(final_point)

    pipe_build_year = point[0]
    pipe_build_month = point[1]
    final_method, final_average_npv = point_simulation(uplifts_count, copy.deepcopy(uplifts), pipe_build_year,
                                                   pipe_build_month, tax_stats, pipe_stats, cars_stats,
                                                   well_stats, copy.deepcopy(recon_results), True)
    print()
    print(final_point)
    print(final_method, final_average_npv)
